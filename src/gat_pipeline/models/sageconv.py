...
class SAGENet(nn.Module):
    def __init__(self, esm_embeds: int, drop_prob: float, n_output: int):
        super().__init__()
        self.gcnconv1 = SAGEConv(in_channels=esm_embeds, out_channels=esm_embeds)
        self.gcnconv2 = SAGEConv(in_channels=esm_embeds, out_channels=esm_embeds)
        self.gcnconv3 = SAGEConv(in_channels=esm_embeds, out_channels=esm_embeds)
